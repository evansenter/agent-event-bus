...
| Wake spool dir | `~/.claude/contrib/agent-event-bus/wake/` (bridge spool/lock files + `panes.json`/`panes.lock` + `<sid>.busy` turn-state markers (mtime is the freshness signal - aged out by `--busy-ttl`) + `bridge.singleton.lock` - transient; the DB protection below does NOT extend to it. Safe to hand-clear **while no bridge is running** - clearing it under a live bridge orphans its `bridge.singleton.lock` inode, so a second instance acquires a fresh one) |
...
**Environment variables**: `AGENT_EVENT_BUS_*` prefix (e.g., `_DB`, `_LOG`, `_ERR`, `_URL`, `_AUTH_DISABLED`, `_ICON`, `_TESTING`, `_SESSION_ID`, `_LOG_PEER`; bridge: `_BRIDGE_PORT`, `_BRIDGE_BACKEND`, `_BRIDGE_COOLDOWN`, `_BRIDGE_BUSY_TTL`, `_BRIDGE_SECRET`, `_BRIDGE_HOOK_URL`, `_BRIDGE_BIND`, `_BRIDGE_ALLOWED_HOSTS`, `_WAKE_DIR`; and `_BRIDGE_LOG` / `_BRIDGE_ERR`, which -
...
├── wake.py        # Wake-dir contract: pane mapping + turn-state markers.
│                  # Imported by BOTH bridge.py (reader) and cli.py (writer)
│                  # on purpose - a drift between them produces no error,
│                  # only wakes that never happen
...
| `panes set/clear`, `wake-state` | *(none)* | Local filesystem only - see below |

`panes` and `wake-state` are deliberately CLI-only: they write the bridge's
wake dir on the local machine, and an MCP tool for them would let any session
on the bus retarget where another machine's keystrokes get typed.
...
- **Deleted publishers are flagged, not rejected (#144)**: the write half goes the other way. `publish_event` under a soft-deleted id still stores the event - keeping its original attribution, not re-writing it to `anonymous` - and adds `session_deleted: true` (no `error` key) to the response. A rejected poll costs one empty loop iteration; a rejected publish loses the event, and the common publishers are fire-and-forget hooks with no retry. Same blast radius rule as #140
- **One contract, three dispositions (#144)**: every session-scoped tool answers a deleted id the same way, so a client branches on `session_deleted` rather than on which tool it called. `_deleted_session_notice(session, tool, disposition=...)` shapes the fields once and *derives* the hint from the disposition rather than taking one - there is no way to hand a publisher "or stop polling" or promise an unregister caller something is left to clean up. `refused` (reads: `error` + warn), `flagged` (publish: no `error`, warn), `already_gone` (`unregister_session`: `error`, **no warn** - unregistering is an orphan winding *down*, the one call from a dead session an operator does not need told, and it does not burn the warn-once slot a later poll would use). Before this, unregister reported `"Session not found"` for a row that exists and that the bus itself deleted
