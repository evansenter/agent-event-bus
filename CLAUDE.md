...
- **Deleted sessions fail loudly (#140)**: `get_events` with a soft-deleted `session_id` returns `{"error": "Session deleted", "session_deleted": true, ...}` on *every* read path, not just `resume`. A deleted session's cursor and heartbeat are both frozen, so an empty batch is indistinguishable from "up to date" while the session is also absent from `list_sessions` - the failure was silent on every axis. Ids never registered here stay silent (foreign session ids are a supported way to read the bus). `ack_events` (#143) and `unregister_session` (#144) share the shape; only reads carry the `error` key
- **Deleted publishers are flagged, not refused (#144)**: `publish_event` from a soft-deleted session **stores the event** and adds `session_deleted`/`display_id`/`deleted_at`/`hint` to its result, with **no `error` key** - the write succeeded, and a caller branching on `error` must not read a stored event as a failed one. Refusing would trade a bad attribution for a *dropped* event, and the publishers that outlive their session are fire-and-forget hooks with no retry. The event keeps its real `session_id` rather than being rewritten to `anonymous`: the sessions row survives soft-deletion, so events still join to it, and the attribution is the only thing that makes an orphaned publisher findable (in the log, and in `events JOIN sessions ON e.timestamp > s.deleted_at`). Costs the publish path one primary-key SELECT it did not make before - `_auto_heartbeat`'s no-op cannot tell "deleted" from "never registered", which is exactly why this was invisible; anonymous publishes skip the lookup entirely
